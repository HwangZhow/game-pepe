...
async function clickConnectTon(runtime) {
	...
	const { isTest, tgSignatrue } = runtime.globalVars
	try {
		 	const { data } =	await request.get("/login/nonce", { headers: {

	"x-telegram": isTest ? tgSignatrue : tgData,
	}})
	const { nonce } = data;
	if (nonce) {
	...
	if (loading?.hasOwnProperty("isVisible")) {
...
const token = localStorage.getItem("token");
  ...
  try {
	...
	text.text = "Success! 20,000 points"
  } catch(e) {
  console.log(e)
...
async function checkIn(runtime) {
   ...
   try {
    const { data } = await request.post("/user/signin",  {}, {
   	...
   	runtime.getInstanceByUid(156).text = "Checked in already!";
	runtime.getLayout("Activity").getLayer(4).isVisible = true;
...
const scriptsInEvents = {
	...
	async PropsEvent_Event9_Act1(runtime, localVars)
	{
		globalThis.getMeInfo(runtime)
	},
	...
	async ActivityEvent_Event1_Act1(runtime, localVars)
	{
	const token = localStorage.getItem("token");
		const tgData = window?.Telegram?.WebApp?.initData;
		//const tgData = "user=%7B%22id%22%3A2007070864%2C%22first_name%22%3A%22gan%22%2C%22last_name%22%3A%22mi%22%2C%22username%22%3A%22kaysfake%22%2C%22language_code%22%3A%22zh-hans%22%2C%22allows_write_to_pm%22%3Atrue%7D&chat_instance=-5093642083902327580&chat_type=sender&auth_date=1721029686&hash=fe01a143c77bc50a4ccc6aeb9824a368d49fde6f631a447e5fb7e1ad0a3ff46b"
	...
	async ActivityEvent_Event3_Act1(runtime, localVars)
	{
		checkIn(runtime)
	},

	async ActivityEvent_Event41_Act1(runtime, localVars)
	{
		clickConnectTon(runtime)
	},

	async ActivityEvent_Event41_Act2(runtime, localVars)
	{
