...
async function getMeInfo (runtime) {
	if (window.axios && window.Telegram) {
		...
		runtime.globalVars.InviteOrNot = data.isInvited;
...
runOnStartup(async runtime => {
	...
	globalThis.tonUI.onStatusChange( async (wallet) => {
      ...
      if (wallet?.account && wallet?.connectItems) {
		...
		const instance = runtime.getInstanceByUid(67);
		
		if (instance?.hasOwnProperty('isVisible')) {
			instance.isVisible = true
		}
		runtime.globalVars.ownton = 1;
		const modalText = runtime.getInstanceByUid(156)
		if (modalText?.hasOwnProperty("text")) {
				modalText.text = "20000D reward"
		}
		
		const layer = runtime.getLayout("Activity").getLayer(4).isVisible = true;
		if (layer?.hasOwnProperty("isVisible")) {
		...
		const modalText = runtime.getInstanceByUid(156)
		if (modalText?.hasOwnProperty("text")) {
				modalText.text = "unable to connect wallet"
		}
	...
	tgSDKScript.onload = async () => {
		const token = localStorage.getItem("token");
			const { isTest, tgSignatrue } = runtime.globalVars
		const tgData = isTest ? tgSignatrue : window.Telegram.WebApp.initData;
